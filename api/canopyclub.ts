...
export default async function handler(_req: VercelRequest, res: VercelResponse) {
  try {
    const response = await fetch('https://www.canopyclub.com/events', {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; HubLive/1.0)' },
    });
    const html = await response.text();
    const events = parseCanopyHTML(html);

    res.setHeader('Cache-Control', 's-maxage=3600, stale-while-revalidate');
    res.status(200).json({ events });
  } catch {
    res.status(500).json({ error: 'Failed to fetch Canopy Club events' });
  }
}

function parseCanopyHTML(html: string): ParsedEvent[] {
  const events: ParsedEvent[] = [];
  const seen = new Set<string>();

  // Try matching event links — Canopy Club uses venue-specific event pages
  const eventLinkRegex = /<a[^>]+href="(https?:\/\/(?:www\.)?canopyclub\.com\/(?:events?\/|show\/)[^"]+)"[^>]*>([^<]{3,80})<\/a>/gi;
  ...
  while ((match = eventLinkRegex.exec(html)) !== null) {
    ...
    // Look for a date in the context
    ...
    const longMatch = dateMatch[0].match(
      /(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2}),?\s*(\d{4})/i
    );
    if (longMatch) {
      const m = months[longMatch[1].toLowerCase()];
      if (m) date = `${longMatch[3]}-${m}-${longMatch[2].padStart(2, '0')}`;
    } else {
      // MM/DD/YYYY
